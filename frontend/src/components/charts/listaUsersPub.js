    ...
    return (
        ...
        >
            <Typography variant="h6" component="div" sx={{ marginBottom: 2 }}>
                Top Publicações por Utilizador:
            </Typography>
            <List>
                {users.map((user, index) => (
                    <ListItem key={index}>
                        ...
                        <ListItemText primary={user.nome} secondary={`${user.threads} publicações`} />
                    </ListItem>
                ))}
            </List>
        </Box>
    ...
    useEffect(() => {
        const fetchDataUsers = async () => {
            try {
                ...
                const response = await axios.get(`${process.env.REACT_APP_API_URL}/utilizadores/utilizadoresthreads`, {
                    headers: {
                        Authorization: token
                    }
                });
