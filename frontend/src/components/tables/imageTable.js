...
const ImageTable = ({ images, onAddImage, styleProp, onDelete, canModify = true }) => {
    return (
        <div style={styleProp}>
            <div style={{ overflowX: 'auto', display: 'flex', whiteSpace: 'nowrap' }}>
                {images.map((image, index) => (
                    <ImageListItem key={index} style={{marginRight: '10px'}}>
                        ...
                        {canModify && (<ImageListItemBar style={{borderRadius: 15, backgroundColor: 'transparent'}}
                            actionIcon={
                ...
                {canModify && (<div style={{ flex: '0 0 200px', height: '200px', border: '1px solid black', display: 'flex', justifyContent: 'center', alignItems: 'center', borderRadius: 15 }}>
