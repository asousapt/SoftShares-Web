...
const NovoDepartamento = ({ open, onClose, setAlertOpen, setAlertProps }) => {
    //VARS
    //FIELDS
    ...
    const handleAddEvent = async () => {
        ...
        try {
            ...
            setAlertProps({ title: 'Erro', label: `Ocorreu um erro ao criar o departamento.`, severity: 'error' });
            setAlertOpen(true);
        }
    };
    ...
    return (
        <Modal open={open} onClose={handleCancel}>
            <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: '800px', maxWidth: '80%', maxHeight: '80%', backgroundColor: '#1D5AA1', padding: '20px' }}>
                <h2 style={{ marginTop: 0, color: 'white' }}>Novo Departamento</h2>
